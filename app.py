...
if all_df.empty:
    ...
    with tab1:
        ...
        for _, row in df_class.iterrows():
            name = row['姓名']
            st.session_state[f'attendance_{selected_class}'][name] = st.checkbox(f"{int(row['座號'])} {name}", value=st.session_state[f'attendance_{selected_class}'][name])
    ...
    with tab2:
        ...
        for name in present_students:
            ...
            col1.write(f"{name} (發言：{st.session_state[f'scores_{selected_class}'][name]})")
    ...
    with tab4:
        ...
        if st.button("💾 儲存今日紀錄"):
            ...
            with st.spinner("同步中..."):
                ...
                requests.post(WEB_APP_URL, json=all_data)
                st.success("✅ 完成！")
